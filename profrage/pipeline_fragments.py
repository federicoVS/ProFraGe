...
from fragment.mine import KSeqMine, KSeqTerMine, HierarchyMine, FuzzleMine
...
def pipeline(method, build, represent, data_dir, out_dir, cmaps, fuzzle_json, k, max_size, cosine_thr, f_thr, ext, verbose=False):
    """
    ...
    cosine_thr : float in [0,1]
        The cosine similarity threshold. A lower threshold implies a higher similitude is needed.
    f_thr : float in [0,1]
    ...
    if method != 'fuzzle':
        ...
        for s_file in s_files:
            ...
            miner = None
            if method == 'kseq':
                miner = KSeqMine(structure, Rep, k, cosine_thr)
            elif method == 'kseqter':
                miner = KSeqTerMine(structure, Rep, k, cosine_thr, cmap_dict[pdb_id]) # CMAP validity already checked
            elif method == 'hierarchy':
                miner = HierarchyMine(structure, builder)
            else:
        ...
        miner = None
        if method == 'fuzzle':
            miner = FuzzleMine(fuzzle_json, verbose=verbose)
        else:
            print('Invaid mining method')
            return
        # Generate the fragments
...
if __name__ == '__main__':
    ...
    arg_parser.add_argument('method', type=str, help='The method to be used to the mine the fragments. The options are [kseq, kseqter, hierarchy, fuzzle].')
    ...
    arg_parser.add_argument('--cmaps', type=str, default='../pdb/cmaps/', help='The directory holding the CMAP files generated by ConFind. This option has effect when methods `kseq`, `kseqter`, and `hierarchy` are chosen. The default is ../pdb/cmaps/.')
    ...
    arg_parser.add_argument('--k', type=int, default=3, help='The number of residues to the left or right of a centroid in a neighborhood. This option has effect when methods `kseq`, `kseqter`, and `hierarchy` are chosen. The default is 3.')
    arg_parser.add_argument('--max_size', type=int, default=3, help='The maximum number of neighborhoods per fragment. This option has effect when methods `kseq`, `kseqter`, and `hierarchy` are chosen. The default is 3.')
    arg_parser.add_argument('--cosine_thr', type=float, default=0.005, help='The cosine threshold. The smaller the more similar. This option has effect when methods `kseq` and `kseqter` are chosen. The default is 0.005.')
    arg_parser.add_argument('--f_thr', type=float, default=0.1, help='The residue-residue interaction score threshold. The larger the more similar. This option has effect when methods `kseq`, `kseqter`, and `hierarchy` are chosen. The default is 0.1.')
    arg_parser.add_argument('--ext', type=str, default='.pdb', help='The format of the input files. The options are [.mmtf, .pdb]. This option has effect when methods `kseq`, `kseqter`, and `hierarchy` are chosen. The deafult is .pdb.')
    ...
    # Begin pipeline
    pipeline(args.method, args.build, args.represent, args.data_dir, args.out_dir, args.cmaps, args.fuzzle_json, args.k, args.max_size, args.cosine_thr, args.f_thr, args.ext, verbose=args.verbose)